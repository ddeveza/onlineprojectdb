...
<style type="text/css">
 ...
 <button type="button" class="btn btn-primary btn-lg project"  
			...
			<div class="modal-content">
			 <form action="" method="post">
			  ...
			  <div class="modal-body" id="projectdetails">
					...
					<textarea name="BusinessCase" style="resize: none; width:500px; height:200px; border:solid 1px orange;"></textarea><br>
				<br>
				<br>
				<table class="table table-striped">
					...
					<option value="releasemethodempty" disabled selected>Selected Release Method</option>
					...
					<td><input type="text" value="<?php echo $_SESSION['TEName']; ?>" id="OSPITE" readonly></input> </td>
					<?php
					...
					<tr>
						...
						<td>
							<input type="text" id="tester" style="text-transform:uppercase"  list='testerlist'></input>
						...
						<td><input type="text"  id="Devicepackage" style="text-transform:uppercase" list="Devicepackagelist"></input>
    ...
    <div class="modal-dialog" role="document">
      ...
      <form action="" method="post" id="updateForm">
        ...
        <div class="modal-footer">
		  ...
		  <button type="button" class="btn btn-warning update" id="<?php echo $ProjectID ;?>"> Update</button>
        </div>
       <form> 
      </div>
    </div>
   ...
   <div class="modal fade" id="DelModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
      ...
      <form action="" method="post" id="updateForm">
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
		  <button type="button" class="btn btn-danger delete" id="<?php echo $ProjectID ;?>"> Delete</button>
        </div>
       <form> 
      </div>
    </div>
	...
	$(document).on('click', '.update', function() {
		...
		if (tester == "" ){alert("Please Enter Tester item!");}
			...
			if (ProjectStatus == "Completed" && (projtype == 'FT NPI' || projtype == 'WS NPI')  ){
				if (prpdate == "" ||qualstartdate == "" ||cabapprovedate == "" ||cabdate==""||ecosubmitteddate == "" ||ecoreleasedddate == "" )
...
$(document).ready(function () {
	...
	$("select#ReleaseMethod").change(function(){
			...
			if (ReleaseMethod == 'N/A') {
					//var Devicepackage =$('#Devicepackage').val();
					$("#projectname").val(DeviceID+" ("+Devicepackage+" "+tester+") "+" - "+" " +projtype );
			}else { 
			...
			if (ReleaseMethod=='RapidRelease'){
				...
				$('<td id="rapid"><label><b>Rapid Release Type of Change:</b></label></td><td  id="rapid"><textarea id="RapidValue"></textarea></td>').insertAfter('#dennis');
			}else{
				$('#rapid').remove();
				$('#rapid').remove();
			}
	...
	if (ReleaseMethod=="releasemethodempty" ){alert("Please Select Release Method Before Edit!");}
		else if (projtype =="projecttypeempty"){ alert("Please Enter Project Type Before Edit!");}
		else if (DeviceID =="deviceidempty"){alert("Please Select Device ID Before Edit! ");}
		else if (tester == ""){alert("Please Input tester Before Edit! ");}
		else if (Devicepackage==""){alert("Please Input Device Package Before Edit");}
		else{$("#projectname").attr("readonly",false);}
	});
...
$(document).ready(function(){
	...
	$('.addproj').on('click',function(event){
		...
		//if (tester == "" ){alert("Please Enter Tester item!");}
		if (businesscase =="" ){alert('Please enter your project business case!');}
		else if (ProjectName==""){ alert("Please Enter Project Name!");}
		else if (projtype =="projecttypeempty"){alert("Please Select Project Type! ");}
		else if (DeviceID =="deviceidempty"){alert("Please Select Device ID! ");}
		else if (ReleaseMethod=="releasemethodempty"){alert("Please Select Release Method or Select Not Applicable!");}
		else if (ProjectStatus=="projectstatusempty"){alert("Please Select Project Status!");}
